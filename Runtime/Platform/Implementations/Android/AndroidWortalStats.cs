...
{
    public class AndroidWortalStats : IWortalStats
    {
        public bool IsSupported => false;

        public void GetStats(string level, GetStatsPayload payload, Action<Stats[]> callback, Action<WortalError> errorCallback)
        {
            Debug.Log($"[Android Platform] IWortalStats.GetStats({level}, {payload}) called - Not implemented");
        ...
        {
            Debug.Log($"[Android Platform] IWortalStats.PostStats({level}, {value}, {payload}) called - Not implemented");
            errorCallback?.Invoke(new WortalError
            {
                Code = "NOT_IMPLEMENTED",
                Message = "Not implemented on Android platform",
